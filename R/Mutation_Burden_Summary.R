...
Output_File <- "J:\\Proj_IO_RIG_NOVA_Grant\\Harmonized_Softlinks\\work\\Alyssa_Work\\WES\\Orien_IO_Rig_VAF_Summary_20240116_v2.txt"
...
header <- c("WES","VCF_Sample_Name","VCF_Exome_Kit",
            ...
            "dN.ds","dN.ds_GreaterThan_3","Aneuploidy_Score","HRD_Score",
...
write(header,file = Output_File, append = T, sep = '\t', ncolumns = 21)
...
for (file in VCF_Anno_files) {
  ...
  HRD_Score <- aneu[which(aneu$WES == WES),"HRD_scarHRD"]
  ...
  Mutation_Burden_Summary <- c(WES,VCF_Sample_Name,VCF_Exome_Kit,
                               ...
                               dN.ds,dN.ds_GreaterThan_3,Aneuploidy_Score,HRD_Score,
                               Total_Mutation_Count,Total_Mutation_Count_Norm,Total_Protein_Impacting_Mutation_Count,Total_Protein_Impacting_Mutation_Count_Norm)
  write(Mutation_Burden_Summary,file = Output_File, append = T, sep = '\t', ncolumns = 21)
